...
bitset<64> column, diagonal1, diagonal2;
char board[64][64];
void NQueen(int row, int &n)
{
    ...
    {
        board[row][c] = ' ';
        if (!column[c] && !diagonal2[row + c] && !diagonal1[n - 1 + row - c])
        {
            column[c] = diagonal2[row + c] = diagonal1[n - 1 + row - c] = 1;
            board[row][c] = 'Q';
            NQueen(row + 1, n);
            column[c] = diagonal2[row + c] = diagonal1[n - 1 + row - c] = 0;
            board[row][c] = ' ';
        }
    }
}
