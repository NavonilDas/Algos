...
int main()
{
    ...
    loop(i, n)
    {
        auto check = points.at(i);
        bool t, l, b, r;
        t = l = b = r = false;
        // cout << "##################" << check.first << "," << check.second << "=>" << endl;
        loop(j, n)
        {
            auto from = points.at(j);
            if (i != j)
            {
                if (check.first < from.first && check.second == from.second)
                {
                    // cout << "right " << from.first << "," << from.second << endl;
                    r = true;
                }
                if (check.first > from.first && check.second == from.second)
                {
                    // cout << "left " << from.first << "," << from.second << endl;
                    l = true;
                }
                if (check.first == from.first && check.second < from.second)
                {
                    // cout << "top " << from.first << "," << from.second << endl;
                    t = true;
                }
                if (check.first == from.first && check.second > from.second)
                {
                    // cout << "bottom " << from.first << "," << from.second << endl;
                    b = true;
                }
            }
            if (t && l && b && r)
