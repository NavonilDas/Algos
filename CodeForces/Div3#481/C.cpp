...
pair<ll, ll> bsearch(vector<pair<ll, ll>> &ar, ll &search)
{
    int l = 0, r = ar.size() - 1;
    while (l <= r)
    {
        int mid = (l + r) >> 1;
        if (ar[mid].first <= search && ar[mid].second >= search)
        {
            return make_pair(mid + 1, search - ar[mid].first + 1);
        }
        if (ar[mid].first > search)
        {
            r = mid - 1;
        }
        else if (ar[mid].second < search)
        {
            l = mid + 1;
        }
    }
...
int main()
{
    ...
    {
        cin >> tmp;
        // auto x = make_pair<ll>(0, tmp);
        if (i > 0)
        {
            ab = make_pair(ab.second + 1, ab.second + tmp);
        }
        else
        {
            ab = make_pair(1, tmp);
