// Har Har Mahadev
#include <bits/stdc++.h>

// #include <ext/pb_ds/assoc_container.hpp>
// using namespace __gnu_pbds;
// #include <boost/multiprecision/cpp_int.hpp>
// using boost::multiprecision::cpp_int;

using namespace std;

#define FASTIO                   \
    ios::sync_with_stdio(false); \
    cin.tie(0);                  \
    cout.tie(0)
#define endl '\n'
#define ll long long
#define ull long long
#define vi vector<int>
#define vt(type) vector<type>
#define mat(type) vector<vector<type>>
#define pb push_back
#define mp make_pair
#define mii map<int, int>
#define setbits(a) __builtin_popcountll(a)
#define zrobits(a) __builtin_ctzll(a)
#define MOD 1000000007
#define INF 1e18
#define pqb priority_queue<int>
#define pqs priority_queue<int, greater<int>>
#define prec(a, b) fixed << setprecision(b) << a
#define till(a) \
    int a;      \
    cin >> a;   \
    while (a--)
#define loop(i, n) for (int i = 0; i < (int)(n); ++i)
#define from(i, a, b) for (int i = (a); i <= (b); ++i)
#define dloop(i, n) for (int i = n; ~i; --i)
#define debug(x1) cout << #x1 << ": " << x1 << endl;
#define all(v) (v).begin(), (v).end()
#define sz(v) ((int)(v.size()))

mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
// typedef tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update> pbds;

// change ull to ll for negative
// ull gcd(ull a,ull b){return (b==0)?a:gcd(b,a%b);}
// ull pwrm(ull a,ull p,ull m){
//     ull res = 1;
//     a = a%m;
//     while(p > 0){
//         if(p&1)
//             res = (res*a)%m;
//         a = (a*a)%m;
//         p >>=1;
//     }
//     return res%m;
// }
// #pragma GCC optimize("Ofast")
// #pragma GCC target("avx,avx2,fma")
// #pragma GCC optimize("unroll-loops")
// fill,copy,lower_bound,upper_bound,max_element,min_element
struct hm
{
    ll a1;
    ll a2;
    ll d;
    ll cost;
};
void addToHsh(vector<hm> &hsh, ll a2, ll a1, ll cst)
{
    hm a;
    a.a1 = a1;
    a.a2 = a2;
    a.cost = cst;
    a.d = a2 - a1;
    hsh.pb(a);
}

int main()
{
    FASTIO;
#ifdef NAVONIL
    clock_t tStart = clock();
    freopen("input.txt", "r", stdin);
    freopen("output.txt", "w", stdout);
#endif
    int n;
    cin >> n;
    vector<ll> a(n);
    loop(i, n)
    {
        cin >> a[i];
    }

    vector<hm> hsh;

    if (n <= 2)
    {
        cout << 0 << endl;
    }
    else
    {
        // Find All Possible d
        // a[1] - (a[0] - 1)
        addToHsh(hsh, a[1], (a[0] - 1), 1);
        // a[1] - a[0]+1 1
        addToHsh(hsh, a[1], (a[0] + 1), 1);
        // a[1] - a[0]
        addToHsh(hsh, a[1], a[0], 0);

        // (a[1]-1) - a[0]
        addToHsh(hsh, (a[1] - 1), a[0], 1);
        // (a[1]-1) - (a[0]-1)
        addToHsh(hsh, (a[1] - 1), (a[0] - 1), 2);
        // (a[1]-1) - (a[0]+1)
        addToHsh(hsh, (a[1] - 1), (a[0] + 1), 2);

        // (a[1]+1) - (a[0]-1)
        addToHsh(hsh, (a[1] + 1), (a[0] - 1), 2);
        // (a[1]+1) - (a[0]+1)
        addToHsh(hsh, (a[1] + 1), (a[0] + 1), 2);
        // (a[1]+1) - a[0]
        addToHsh(hsh, (a[1] + 1), a[0], 1);
        bool ans = false;
        ll finalCost = -1;

        for (auto &dist : hsh)
        {
            ll cost = dist.cost;
            ll a1 = dist.a1;
            ll d = dist.d;
            bool flag = true;
            // cout << endl
            //      << a1 << " " << dist.a2 << " \n\n";
            // cout << dist.d << " " << dist.cost << endl
            //      << endl;
            from(i, 2, n - 1)
            {
                ll an = a1 + i * d;
                // cout << an << "  " << a[i] << "  " << abs(an - a[i]) << endl;
                if (abs(an - a[i]) > 1)
                {
                    flag = false;
                    break;
                }
                else
                    cost += abs(an - a[i]);
            }
            if (flag)
            {
                if (finalCost == -1)
                    finalCost = cost;
                else
                    finalCost = min(finalCost, cost);
                ans = true;
            }
        }
        if (!ans)
        {
            cout << -1 << endl;
        }
        else
        {
            cout << finalCost << endl;
        }
    }
#ifdef NAVONIL
    printf("Time taken: %.2fs\n", (double)(clock() - tStart) / CLOCKS_PER_SEC);
#endif
    return 0;
}